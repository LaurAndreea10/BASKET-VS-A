(() => {
  ...
  function loop(now){ const dt = Math.min(.033, (now-last)/1000 || .016); last = now; time = now / 1000; if(!over && match.mode !== 'practice'){ match.left -= dt; if(match.left <= 0) return finish(); if($('#timer')) $('#timer').textContent = Math.ceil(match.left); } if(match.mode === 'wind' || match.mode === 'bossrush'){ const shield = 1 - progress.upgrades.wind * .13; match.wind = Math.sin(time * 1.7) * 70 * shield; if($('#windTag')) $('#windTag').textContent = 'Vânt ' + Math.round(match.wind); } update(dt); render(); raf = requestAnimationFrame(loop); }
  ...
  if($('#reset')) $('#reset').onclick=()=>{ if(confirm('Resetezi progresul?')){ localStorage.removeItem(STORE); progress=clone(baseProgress); save(); location.reload(); } };
