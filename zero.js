(() => {
  'use strict';
  const VERSION = 'zero-final-v5-hubfix';
  ...
  const match = { mode:'classic', left:60, you:0, ai:0, wind:0, combo:0, started:false, playerPerfects:0, playerBankShots:0 };
  ...
  function loadProgress(){ try { return deepMerge(clone(baseProgress), JSON.parse(localStorage.getItem(STORE) || 'null') || {}); } catch { return clone(baseProgress); } }
  function loadPack(){ try { return deepMerge(clone(basePack), JSON.parse(localStorage.getItem(PACK_STORE) || 'null') || {}); } catch { return clone(basePack); } }
  function savePack(pack){ localStorage.setItem(PACK_STORE, JSON.stringify(pack)); }
  function packSkills(){ return loadPack().skills || basePack.skills; }
  function skill(key){ return Number(packSkills()[key] || 0); }
  ...
  function mode(){ return $('input[name="mode"]:checked')?.value || 'classic'; }
  function dur(){ return +($('input[name="dur"]:checked')?.value || 60); }
  ...
  function perfectBand(){ const extra = progress.upgrades.assist * .012 + skill('precision') * .014; return [.47-extra, .57+extra]; }
  ...
  function currentOpponent(){ return match.mode === 'bossrush' ? opponents[progress.bossStage % opponents.length] : opponents[Math.min(progress.tournamentRound, opponents.length-1)]; }
  ...
  function openHub(){ const hub = $('#hub'); if(!hub) return; hub.classList.add('open'); hub.style.display = 'block'; syncHub(); hub.scrollIntoView({ block:'start', behavior:'smooth' }); }
  function closeHub(){ const hub = $('#hub'); if(!hub) return; hub.classList.remove('open'); hub.style.display = ''; }
  window.openBVAIHub = openHub;
  window.closeBVAIHub = closeHub;
  ...
  function updateReadouts(){ const pct = powerPercent(), angle = aimAngle(), status = shotStatus(); if($('#fill')) $('#fill').style.width = pct + '%'; if($('#powerReadout')) $('#powerReadout').textContent = `${pct}% · ${status}`; if($('#dataPower')) $('#dataPower').textContent = pct + '%'; if($('#dataAngle')) $('#dataAngle').textContent = angle + '°'; if($('#dataStatus')) $('#dataStatus').textContent = status; if($('#aimReadout')) $('#aimReadout').textContent = angle + '°'; if($('#perfectText')) $('#perfectText').style.opacity = isPerfectPower() ? '1' : '.55'; }

  function start(){
    cancelAnimationFrame(raf); resetDailyIfNeeded(); match.mode = mode(); match.left = match.mode === 'practice' ? 999 : match.mode === 'blitz' ? 30 : dur();
    match.you = 0; match.ai = 0; match.wind = 0; match.combo = 0; match.playerPerfects = 0; match.playerBankShots = 0; match.started = true; balls = []; particles = []; over = false; charge = 0; charging = false; aiCooldown = 1.2;
    $('#end')?.classList.remove('show'); if($('#scoreP')) $('#scoreP').textContent = 0; if($('#scoreAI')) $('#scoreAI').textContent = 0; if($('#timer')) $('#timer').textContent = match.mode === 'practice' ? '∞' : match.left;
    const opp = currentOpponent(); if($('#aiName')) $('#aiName').textContent = match.mode === 'practice' ? 'Practice' : opp.name; if($('#modeTag')) $('#modeTag').textContent = `${label(match.mode)} · ${$('#aiName')?.textContent || opp.name}`; if($('#windTag')) $('#windTag').style.display = (match.mode === 'wind' || match.mode === 'bossrush') ? 'block' : 'none';
    updateReadouts(); syncHub(); last = performance.now(); raf = requestAnimationFrame(loop);
  }
  function loop(now){ const dt = Math.min(.033, (now-last)/1000 || .016); last = now; time = now / 1000; if(!over && match.mode !== 'practice'){ match.left -= dt; if(match.left <= 0) return finish(); if($('#timer')) $('#timer').textContent = Math.ceil(match.left); } if(match.mode === 'wind' || match.mode === 'bossrush'){ const shield = Math.max(.2, 1 - progress.upgrades.wind * .13 - skill('focus') * .10); match.wind = Math.sin(time * 1.7) * 70 * shield; if($('#windTag')) $('#windTag').textContent = 'Vânt ' + Math.round(match.wind); } update(dt); render(); raf = requestAnimationFrame(loop); }
  ...
  function createShot(owner, target, power, angleDeg, forcedPerfect = false){ const perfect = forcedPerfect || isPerfectPower(power); const angleError = (angleDeg - 8) * 1.25; const powerError = (power - .52) * 150; const assist = progress.upgrades.assist * 2 + skill('precision') * 2.4; const missScale = perfect ? 0 : 1; const targetX = target.rimX + missScale * (angleError + powerError * .20 - assist * Math.sign(angleError || 1)); const targetY = target.rimY + missScale * Math.abs(powerError) * .07; const dx = targetX - owner.ballX, dy = targetY - owner.ballY; const frames = 54, gravity = .22; balls.push({ x:owner.ballX, y:owner.ballY, vx:dx/frames, vy:(dy - .5*gravity*frames*frames)/frames, g:gravity, rot:0, target, owner, perfect, trail:[] }); }
  function update(dt){
    if(charging){ const speed = Math.max(1.65, 4.2 - progress.upgrades.control * .36 - skill('stamina') * .30); charge = (Math.sin(time * speed - Math.PI/2) + 1) / 2; updateReadouts(); }
    ...
    for(let i=balls.length-1;i>=0;i--){ const b = balls[i]; b.vx += match.wind * .0016; b.x += b.vx; b.y += b.vy; b.vy += b.g; b.rot += .25; b.trail.push({x:b.x,y:b.y}); if(b.trail.length > 14) b.trail.shift(); const dx = b.x - b.target.rimX, dy = b.y - b.target.rimY; if(Math.abs(dx)<32 && dy>-8 && dy<16 && b.vy>0 && !b.hit){ b.hit = true; if(Math.abs(dx)<20 && b.perfect){ boom(b.target.rimX,b.target.rimY,'perfect'); if(b.owner === player) scorePlayer(); else { match.ai += 2; if($('#scoreAI')) $('#scoreAI').textContent = match.ai; } balls.splice(i,1); continue; } else { if(b.owner === player){ match.combo = 0; if(match.mode === 'sudden') finish(); } b.vx *= -.35; b.vy *= -.55; boom(b.x,b.y,'miss'); } } if(b.y > H + 60 || b.x < -80 || b.x > W + 80) balls.splice(i,1); }
  }
  function updatePackAfterPerfect(){ const pack = loadPack(); pack.weekly = pack.weekly || { wins:0, perfect:0, boss:0 }; pack.weekly.perfect = Math.min(30, (pack.weekly.perfect || 0) + 1); if(match.combo >= 5) pack.skillPoints = Math.max(pack.skillPoints || 0, Math.floor(progress.level / 2) + 1); savePack(pack); }
  function scorePlayer(){ let pts = match.mode === 'bank' ? 2 + progress.upgrades.bank + skill('bank') : 2; if((progress.upgrades.clutch || skill('clutch')) && match.left <= 10 && match.mode !== 'practice') pts *= (2 + Math.min(1, skill('clutch') * .2)); match.you += Math.round(pts); match.combo++; match.playerPerfects++; progress.perfect++; progress.comboMax = Math.max(progress.comboMax, match.combo); progress.best = Math.max(progress.best, match.you); if(match.mode === 'bank'){ match.playerBankShots++; progress.bank++; progress.missions.bank = Math.min(3, progress.missions.bank + 1); } if(match.combo >= 5) progress.missions.combo = 5; updatePackAfterPerfect(); if($('#scoreP')) $('#scoreP').textContent = match.you; save(); }
  function finish(){ over = true; cancelAnimationFrame(raf); const win = match.you >= match.ai; let reward = win ? 90 : 20; reward += skill('style') * (win ? 18 : 7); if(win && match.mode !== 'practice'){ progress.wins++; progress.money += reward; progress.xp += 45; progress.fans += 5 + skill('style'); const pack = loadPack(); pack.weekly = pack.weekly || { wins:0, perfect:0, boss:0 }; pack.weekly.wins = Math.min(10, (pack.weekly.wins || 0) + 1); if(match.mode === 'bossrush') { progress.bossStage = (progress.bossStage + 1) % opponents.length; pack.weekly.boss = Math.min(1, (pack.weekly.boss || 0) + 1); } else progress.tournamentRound = Math.min(opponents.length - 1, progress.tournamentRound + 1); if(progress.level >= 15) pack.finalChampionshipUnlocked = true; if(progress.level >= 15 && match.mode === 'bossrush') pack.finalChampionDefeated = true; pack.skillPoints = Math.max(pack.skillPoints || 0, Math.floor((progress.level + 1) / 2)); savePack(pack); if(match.you - match.ai >= 10) progress.missions.margin = 1; if(match.mode === 'bank' || match.mode === 'wind') progress.missions.hard = 1; } else if(match.mode !== 'practice') { progress.money += reward; progress.xp += 15; } while(progress.xp >= 100){ progress.xp -= 100; progress.level++; progress.money += 50; const pack = loadPack(); pack.skillPoints = (pack.skillPoints || 0) + 1; if(progress.level >= 15) pack.finalChampionshipUnlocked = true; savePack(pack); } save(); if($('#endTitle')) $('#endTitle').textContent = win ? 'VICTORIE!' : 'ÎNFRÂNGERE'; if($('#endText')) $('#endText').textContent = `Scor final: Tu ${match.you} - ${match.ai} ${$('#aiName')?.textContent || ''}. +${Math.round(reward)} bani`; $('#end')?.classList.add('show'); }
  ...
  function syncHub(){ const setK=(name,val)=>$$('.kpi').forEach(k=>{ if((k.querySelector('span')?.textContent||'')===name) k.querySelector('b').textContent=val; }); setK('Nivel',progress.level); setK('Bani',progress.money); setK('Fani',progress.fans); setK('Best',progress.best); setK('Perfecte',progress.perfect); setK('Bank',progress.bank); setK('Combo max',progress.comboMax); if($('#kBest')) $('#kBest').textContent=progress.best; if($('#kPerfect')) $('#kPerfect').textContent=progress.perfect; if($('#kBank')) $('#kBank').textContent=progress.bank; if($('#kCombo')) $('#kCombo').textContent=progress.comboMax; const idCard=$('.identityFloat'); if(idCard){ idCard.querySelector('p').textContent=`Nivel ${progress.level} • ${rank()}`; const xp=idCard.querySelector('.row b'); if(xp) xp.textContent=`${progress.xp} / 100`; const bar=idCard.querySelector('.xpbar i'); if(bar) bar.style.width=progress.xp+'%'; } fillShop(); fillComp(); }
  function fillShop(){ if(!$('#shopGrid')) return; $('#shopGrid').innerHTML = shopItems.map(item => { if(item.type==='upgrade'){ const lv=progress.upgrades[item.key]||0, maxed=lv>=item.max, price=item.price+lv*60; return `<div class="card"><strong>${item.title}${lv}</strong><p>${item.desc}</p><div class="row"><span class="tag">Lv ${lv}/${item.max} · ${price}$</span><button data-buy="${item.key}" ${maxed?'disabled':''}>${maxed?'MAX':'Cumpără'}</button></div></div>`; } const owned=!!progress.owned[item.key], eq=progress.equipped[item.slot]===item.key; return `<div class="card"><strong>${item.title}</strong><p>${item.desc}</p><div class="row"><span class="tag">${owned?'Owned':item.price+'$'}</span><button data-skin="${item.key}" data-slot="${item.slot}">${eq?'Echipat':owned?'Echipează':'Cumpără'}</button></div></div>`; }).join(''); }
  function fillComp(){ const comp=$('#comp'); if(!comp) return; const r=progress.tournamentRound; const pack = loadPack(); const mission=(label,cur,max,reward)=>`<div class="mission">${cur>=max?'✅':'⬜'} ${label} <span class="tag">${cur}/${max} · +${reward} bani</span><div class="bar"><i style="width:${Math.min(100,cur/max*100)}%"></i></div></div>`; comp.innerHTML=`<div class="section"><h3>Daily Missions</h3><div class="grid">${mission('Marchează 3 bank shots',progress.missions.bank,3,90)}${mission('Câștigă un meci pe Greu',progress.missions.hard,1,120)}${mission('Fă combo x5',progress.missions.combo,5,110)}${mission('Bate AI-ul la 10 puncte diferență',progress.missions.margin,1,160)}</div></div><div class="section"><h3>Weekly Missions</h3><div class="grid">${mission('Câștigă 10 meciuri',pack.weekly?.wins||0,10,500)}${mission('30 aruncări perfecte',(pack.weekly?.perfect||0),30,700)}${mission('Învinge un boss',(pack.weekly?.boss||0),1,450)}</div></div><div class="section"><h3>Turneu</h3><div class="grid">${opponents.slice(0,3).map((o,i)=>`<div class="card"><strong>${i<r?'✅':i===r?'▶':'🔒'} Runda ${i+1} — ${o.name}</strong><p>${i<r?'Câștigată':i===r?'Curentă':'Blocată'}</p></div>`).join('')}</div></div><div class="section"><h3>Adversari AI</h3><div class="grid">${opponents.map(o=>`<div class="card"><strong>${o.name}</strong><p>${o.style}</p><span class="tag">${o.id}</span></div>`).join('')}</div></div><div class="section"><h3>Boss Rush</h3><p>Rookie Bot → Sniper AI → Bank Boss → Legend AI</p><button class="primary" id="bossBtn">Joacă următorul meci</button></div>`; const boss=$('#bossBtn'); if(boss) boss.onclick=()=>{ $('input[value="bossrush"]').checked=true; start(); scrollTo({top:arena.offsetTop-10,behavior:'smooth'}); }; }
  ...
  document.addEventListener('click', e => {
    const hubButton = e.target.closest('#openHub,[data-open-hub]');
    if(hubButton){ e.preventDefault(); openHub(); return; }
    if(e.target.closest('#closeHub,[data-close-hub]')){ e.preventDefault(); closeHub(); return; }
    const tab = e.target.closest('.tab[data-tab]');
    if(tab){ $$('.tab').forEach(x=>x.classList.remove('active')); $$('.panel').forEach(p=>p.classList.remove('active')); tab.classList.add('active'); $('#'+tab.dataset.tab)?.classList.add('active'); syncHub(); return; }
    const b=e.target.closest('[data-buy],[data-skin]'); if(b) buy(b.dataset.buy||b.dataset.skin);
  });

  if($('#endHub')) $('#endHub').onclick=()=>{ openHub(); $('#end')?.classList.remove('show'); };
  if($('#reset')) $('#reset').onclick=()=>{ if(confirm('Resetezi progresul?')){ localStorage.removeItem(STORE); localStorage.removeItem(PACK_STORE); progress=clone(baseProgress); save(); location.reload(); } };
  if($('#play')) $('#play').onclick=()=>{ start(); scrollTo({top:arena.offsetTop-10,behavior:'smooth'}); };
  if($('#restart')) $('#restart').onclick=start; if($('#again')) $('#again').onclick=start; if($('#next')) $('#next').onclick=()=>start();
  if($('#shoot')) $('#shoot').onpointerdown=beginCharge; addEventListener('pointerup',releaseShot); if($('#aim')) $('#aim').oninput=updateReadouts; $$('input[name="mode"],input[name="dur"]').forEach(i=>i.onchange=start);
